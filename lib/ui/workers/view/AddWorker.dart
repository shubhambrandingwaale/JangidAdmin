...
class AddNewWorkerController extends StatelessWidget {
  AddNewWorkerController(
      {Key? key, required this.iscomeFromHomeScreen, this.workerid = 0})
      : super(key: key);
...
class AddNewWorker extends StatefulWidget {
  AddNewWorker(
      {super.key, required this.iscomeFromHomeScreen, this.workerid = 0});
  bool iscomeFromHomeScreen;
  int workerid;
...
class _AddNewWorkerState extends State<AddNewWorker>
  ...
  void initState() {
    Future.delayed(const Duration(milliseconds: 10), () async {
      ...
      if (widget.iscomeFromHomeScreen) {
        ...
        auth.getWorkerById(context, widget.workerid).asStream().listen((event) {
          auth.nameController.text = auth.workerIndividualData!.fullname ?? '';
          auth.userNameController.text =
              auth.workerIndividualData!.username ?? '';
          auth.dailyWagesController.text =
              '${auth.workerIndividualData!.dailyWageSalary}' ?? '';
          auth.contactNumberController.text =
              auth.workerIndividualData!.phone ?? '';
          auth.passwordController.text =
              auth.workerIndividualData!.password ?? '';
          auth.addressController.text =
              auth.workerIndividualData!.address ?? '';
          // if (auth.workerIndividualData!.siteAssigned.isNotEmpty) {
          //   Customlog(' id id : ${auth.workerIndividualData!.siteAssigned}');
          //   auth.selectedSiteId = auth.workerIndividualData!.siteAssigned;
          // }
          // auth.selectedSiteId = auth.workerIndividualData!.siteAssigned != ""
          //     ? auth.workerIndividualData!.siteAssigned
          //     : 'null';
    ...
    if (pickedImages != null && pickedImages.isNotEmpty) {
      if (!widget.iscomeFromHomeScreen) {
        ...
        auth.UpdateDocuments(
            context, myDocumentfiles, widget.workerid.toString());
      } else {
        setState(() {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                          ...
                          children: [
                            Expanded(
                                child: Padding(
                                    padding: const EdgeInsets.all(8.0),
                                    child: CustomTextFeild(
                                        HintText: 'Daily Wage Salary',
                                        Radius: 5,
                                        width: size.width * 0.40,
                                        TextInputAction: TextInputAction.done,
                                        keyboardType: TextInputType.number,
                                        controller: auth.dailyWagesController,
                                        onchangeFuntion: auth.dailywagesError,
                                        errortext: auth.dailyWagesError,
                                        obscureText: false,
                      ...
                      Visibility(
                        visible: auth.workerIndividualData != null &&
                            auth.workerIndividualData!.docs != null &&
                            auth.workerIndividualData!.docs.isNotEmpty,
                        child: SizedBox(
                          height: 200, // Adjust the height as per your design
                          child: ListView(
                            scrollDirection: Axis.horizontal,
                            children: auth.workerIndividualData == null
                                ? []
                                : auth.workerIndividualData!.docs.map((url) {
                                    String updatedUrl =
                      ...
                      MyAnimatedbutton(
                          ...
                          onPressed: () async {
                            ...
                            if (widget.iscomeFromHomeScreen) {
                              await auth.addWorkerValidation(
                                  context,
                                  auth.myfiles,
                                  true,
                                  widget.workerid.toString());
                              ...
                              auth.addWorkerValidation(context, auth.myfiles,
                                  false, widget.workerid.toString());
                            }
                          })
                    ],
                  ),
  ...
  userImage(File image) {
    ...
    if (image != null) {
      ...
      if (auth.workerIdBySite.isEmpty) {
        auth.uploadPic(context, image, widget.workerid.toString(), false);
        Customlog('image  is this $imagePath');
      } else {
        Navigator.pop(context);
        auth.status == Status.Authenticating;
        auth.uploadPic(context, image, auth.workerIdBySite, true);
        Customlog('image  is this $imagePath');
      }
    }
    setState(() {
  ...
  Widget imagewithicon(WorkerRepository auth, String? from) {
    return Container(
      height: 100,
      width: 100,
      decoration: BoxDecoration(
          ...
          Positioned(
            ...
            child: auth.isprofileupdateStart
                ...
                : imageFile != null
                    ...
                    : from == 'profile'
                        ? cacheImage(
                            auth.workerIndividualData?.profileImg != null
  ...
  Future<void> _uploadPictureDialog(
      WorkerRepository auth, BuildContext context) async {
